...
struct PizzaFactoryInterface {
    ...
    virtual shared_ptr<CrustInterface>   make_crust   () const = 0;
    virtual shared_ptr<SauceInterface>   make_sauce   () const = 0;
    virtual shared_ptr<ToppingInterface> make_topping () const = 0;};

struct ChicagoPizzaFactory : PizzaFactoryInterface {
    shared_ptr<CrustInterface> make_crust () const {
        return make_shared<ThickCrust>();}

    shared_ptr<SauceInterface> make_sauce () const {
        return make_shared<MushroomSauce>();}

    shared_ptr<ToppingInterface> make_topping () const {
...
int main () {
    cout << "AbstractFactoryPattern2.c++" << endl;
